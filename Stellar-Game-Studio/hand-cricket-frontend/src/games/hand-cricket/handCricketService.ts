import { Client as HandCricketClient, type Game } from './bindings';
...
/**
 * Service for interacting with the HandCricket game contract
 */
export class HandCricketService {
  private baseClient: HandCricketClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new HandCricketClient({
  ...
  private createSigningClient(
    ...
    return new HandCricketClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new HandCricketClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
...
// Example: const handCricketService = new HandCricketService(contractId);
